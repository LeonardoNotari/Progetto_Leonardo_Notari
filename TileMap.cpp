...
bool TileMap::loadMap(sf::Vector2u tileSize, unsigned int width, unsigned int height,std::vector<Tile *> &tiles) {
    ...
    for (unsigned int i= 0; i < height; i++) {
        for (unsigned int j = 0; j < width; j++) {
            ...
            quad[0].position = sf::Vector2f(tile->xVertexTopDx, tile->yVertexTopDx);
            quad[1].position = sf::Vector2f(tile->xVertexTopDx + 64, tile->yVertexTopDx);
            quad[2].position = sf::Vector2f(tile->xVertexTopDx + 64, tile->yVertexTopDx + 64);
            quad[3].position = sf::Vector2f(tile->xVertexTopDx, tile->yVertexTopDx + 64);
